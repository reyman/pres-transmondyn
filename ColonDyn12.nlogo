...
to setup-globals
  ...
  set energy-level-of-groups-at-start 1100 
end

to setup-patch-ressource
  ;; affect ressources
 if Resource_Landscape = "Homogeneous"  [ask patches [set p_ressource 100]]
 if Resource_Landscape = "Gradient" [ask patches [set p_ressource 50 + abs (min-pycor - pycor)]]
 if Resource_Landscape = "Random" [
    ask patches [set p_ressource random 100]
    repeat 63 [diffuse p_ressource 0.50]
 ]
 if Resource_Landscape = "U-turn" [
    ask patches [
...
to test-migration
 ...
 ifelse  ressource-around < human-pressure-on-ecology
   [migrate-group]
...
to exploit-ressource [my-patches stock-around]
   ...
   let ressource-exploited min (list (human-pressure-on-ecology) stock-around) ;;on exploite "Ressource-factor" unités par habitant au maximum
...
if migration-rule = "Minimisation of distance"
  [set destination destination-taking-into-account-territory]
...
to-report destination-taking-into-account-territory
  ;; 
  report min-one-of patches with [not any? other groups in-radius distance-exploitation]  [distance myself]
  
end

to-report destination-unoccupiedland
  report min-one-of patches with [p_ressource =  p_max-ressource and not any? other groups in-radius distance-exploitation]  [distance myself]
end

to-report destination-maximumressource
  report max-one-of patches in-radius (2 * distance-exploitation) with [not any? other groups in-radius distance-exploitation]  [p_ressource] 
end
